...
// 工作模式
typedef enum {
  LIS3DH_POWER_DOWN     =0x00,  //掉电
  LIS3DH_LOW_POWER 	    =0x01,  //低功耗
  LIS3DH_NORMAL			=0x02   //正常
} LIS3DH_Mode_t;

// 滤波器配置
typedef enum {
  LIS3DH_HPM_NORMAL_MODE_RES        =0x00,  //正常模式（复位读取HP_RESET_FILTER）
  LIS3DH_HPM_REF_SIGNAL             =0x01,  //滤波参考信号
  LIS3DH_HPM_NORMAL_MODE            =0x02,  //正常模式
  LIS3DH_HPM_AUTORESET_INT          =0x03   //中断事件自动复位
} LIS3DH_HPFMode_t;
...
// FIFO 模式选择
typedef enum {
  LIS3DH_FIFO_BYPASS_MODE           =0x00,  //Bypass 模式
  LIS3DH_FIFO_MODE                  =0x01,  //FIFO 模式
  LIS3DH_FIFO_STREAM_MODE           =0x02,  //Stream 模式
  LIS3DH_FIFO_TRIGGER_MODE          =0x03,  //Stream-to-FIFO 模式
...
status_t LIS3DH_Int2LatchEnable(State_t latch);
...
typedef status_t (* i2c_write_callback_t)(u8_t, u8_t const *, u8_t);
typedef status_t (* i2c_read_callback_t)(u8_t, u8_t *, u8_t);

void lis3dh_regist_init(i2c_write_callback_t i2c_write_callback, i2c_read_callback_t i2c_read_callback);
