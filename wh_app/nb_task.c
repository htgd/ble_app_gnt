...
void nb_report_thread(void *pvParameters)
{
    ...
    while(1)
    {
        uxBits = xEventGroupWaitBits(GntEventGroup, 
                                    KEY_EVENTBIT|KEY_LONGPUSH_EVENTBIT|LISINT1_EVENTBIT|LISINT2_EVENTBIT|NB_PRERIOD_REPORT_EVENTBIT|GNT_CLEAR_MOVEFLAG_EVENTBIT, 
        ...
        if((uxBits & NB_PRERIOD_REPORT_EVENTBIT) == NB_PRERIOD_REPORT_EVENTBIT)   //NB周期上报事件
        {
            ...
            if(g_gnt_info.gnt_battery_level > GNT_REPORT_DISABLE_BATTERY_THRESHOLD) //电量低时取消上报
            {
				...
				snprintf(temp, 100, "@03+%02d+%03d+%1d%1d+%11.11s+%1.1s+%11.11s+%1.1s^",     \
                    g_gnt_info.gnt_battery_level, gnt_temp_get(), g_gnt_info.gnt_usbvin_flag, g_gnt_flash_info.gnt_move_alarm_flag,     \
                    g_gnt_info.gnt_last_latitude, &g_gnt_info.gnt_last_latitude[11], g_gnt_info.gnt_last_longitude, &g_gnt_info.gnt_last_longitude[11]); //上报电量、温度
				
				Ble_log_send((uint8_t*)temp, strlen(temp)); //蓝牙监测
				
                // 上报+主动释放RCC+更新注册
                if(g_me3616_info.gnss_run_state == 0)    //未注册成功，上报需先注册
                {
                    nb_handle.nb_fxnTablePtr->nbNotify(&nb_handle, (uint8_t*)temp, strlen(temp), NB_RRC_DISABLE, NB_UPDATE_ENABLE, NB_REPFAIL_REG_TRY);
                }
                else if(g_me3616_info.gnss_run_state == 1)
                {
                    nb_handle.nb_fxnTablePtr->nbNotify(&nb_handle, (uint8_t*)temp, strlen(temp), NB_RRC_DISABLE, NB_UPDATE_ENABLE, NB_REPFAIL_REG_NO_TRY);
                }
        ...
        if((uxBits & LISINT2_EVENTBIT) == LISINT2_EVENTBIT)     //加速度传感器运动检测事件
        {
#if (GNT_GPS_EN == 1)
            g_gnt_info.gnt_battery_level = battery_adc_sample();
            if((g_gnt_info.gnt_gps_battery_low_flag == 0) && (g_gnt_info.gnt_battery_level < GNT_GPS_DISABLE_BATTERY_THRESHOLD))
            {
                g_gnt_info.gnt_gps_battery_low_flag = 1;
            }
            else if((g_gnt_info.gnt_gps_battery_low_flag == 1) && (g_gnt_info.gnt_battery_level > GNT_GPS_RESUME_BATTERY_THRESHOLD))
            {
                g_gnt_info.gnt_gps_battery_low_flag = 0;
            }
            
            if(g_gnt_info.gnt_gps_battery_low_flag == 0)        //电量大于15才启动GPS
            {
                nb_handle.nb_fxnTablePtr->gpsRun(&nb_handle);  
            }                
            
            Ble_log_send((uint8_t*)"int", 3);  //蓝牙监测
            
            vTaskDelay(5000);
#endif
            
//            taskENTER_CRITICAL();       //进入临界区保证lis3dh中断清除OK
//            lis3dh_ResetInt1Latch(3);    //清中断锁存
//            taskEXIT_CRITICAL();
            
            NRF_LOG_INFO("INT2 clear");
        }
        if((uxBits & LISINT1_EVENTBIT) == LISINT1_EVENTBIT)
        {
//            vTaskDelay(2000);
//            LIS3DH_ResetInt2Latch();
//            NRF_LOG_INFO("INT1 clear");
        }
...
//            nb_handle.nb_fxnTablePtr->gpsRun(&nb_handle);
        ...
        {
//			nRF52_FxnTable.bleDisconnect(); 	//断开蓝牙连接
