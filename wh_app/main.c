...
* </table>
* @section   项目详细描述
* 亨通资产电子标签采用有源GPS 定位器和NB-Io 通信网，将资产的移动情况随时上报网管，并提供资产的远程自动清查能
* 力。这对全面、准确的资产管理带来了便利，能够极大地提高资产清查效率，减少资产的浪费和重复投资。
*
* @section   功能描述  
* -# 本工程基于蓝牙新品nRF52832开发
* -# 本工程基于蓝牙协议栈开发，协议栈版本 SDK-15.0
* -# 资产电子标签采用内置GNSS定位功能的NB-IoT模组为ME3616-G1A
* -# 电子标签内置GPS 接收机，在标签移动过程中随时采集标签随资产移动的进程，了解资产的安置地址
* -# GIS 功能，便于在资产清查和查找时进行的导航和全局资产位置的查看
* -# NB-Iot 通信技术，具有极低的待机电流和良好的通信灵敏度
* 
* @section   用法描述 
* -# 蓝牙程序升级操作指南
* -# 资产电子标签生产测试指导
...
/**@defgroup app_gnt Wh app_main
* @{
*
* @brief 资产电子标签蓝牙应用主程序入口.
...
#if (GNT_BLE_SEC_MITM_EN)
#define SEC_PARAM_MITM                      1                                       /**< Man In The Middle protection not required. */
#define SEC_PARAM_IO_CAPABILITIES           BLE_GAP_IO_CAPS_DISPLAY_ONLY            ///< 需要显示
#else
...
static void gnt_ble_disconnect(void);
/** @} 函数前置定义 */
...
BLE_FxnTable nRF52_FxnTable = {
	...
	.bleDisconnect			= gnt_ble_disconnect,
};
...
#if (GNT_BLE_SEC_MITM_EN) 
static ble_opt_t m_static_pin_option;   ///< 静态密钥设置结构体
#endif

/**@brief 通用访问规范（GAP）初始化
 *
...
#if (GNT_BLE_SEC_MITM_EN)    
    // 添加静态密钥到gap
    uint8_t passkey[] = BLE_STATIC_PASSKEY; 
...
static void app_ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        {
#if (GNT_BLE_SEC_MITM_EN)
            ble_gap_sec_params_t temp_params;
            temp_params.bond = 1;
            temp_params.mitm = 1;
            sd_ble_gap_authenticate(m_conn_handle, &temp_params);   //发送安全请求
#endif
...
#if (GNT_BLE_SEC_MITM_EN)
        case BLE_GAP_EVT_SEC_PARAMS_REQUEST:    // 配对请求
        {
            resp_pair_request();
        } break;
        
        case BLE_GAP_EVT_AUTH_STATUS:           // 判断配对是否成功，如果不成功断开连接，从而阻止他人任意连接
        {
...
//                if(g_gnt_flash_info.gnt_enable_flag == GNT_ENABLE)
//                    nRF52_FxnTable.bleSetName(g_gnt_flash_info.ble_name, 9);    //缩短蓝牙名称以区分
//                else if(g_gnt_flash_info.gnt_enable_flag == GNT_DISABLE)
//                    nRF52_FxnTable.bleSetName(g_gnt_flash_info.ble_name, 9);
...
int main(void)
{
    ...
    NRF_LOG_INFO("GNTS FreeRTOS APP started.");
...
#if (IODM_CMD_TEST_EN == 1)
	vTaskSuspend(m_nb_thread);
	nRF52_FxnTable.bleSetName("iODS123", 7);
#endif
