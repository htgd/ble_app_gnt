...
static void ble_comm_handle(uint8_t * Data,uint16_t len)
{
    ...
    switch((uint16_t)(((uint16_t)(Data[8]<<8))|Data[7]))
    {
        ...
        case 0x7103:    //读标签告警和状态（电量+注册状态+搬动告警）
        {
...
//                NRF_LOG_INFO("iot_regist_status = %d ",g_me3616_info.iot_regist_status);
                ...
                #if (GNT_GPS_EN == 1)
                if(g_gnt_flash_info.gnt_move_alarm_flag == 1)
                    Ble_rply[12] = 0xFF;
                else
                    Ble_rply[12] = 0x00;
                //资产搬动是否超过50m
...
//            NRF_LOG_INFO("nb rssi = %d.", Ble_rply[10]);
        ...
        case 0x7109:    //清除标签移动标志
        {
            if(g_gnt_flash_info.gnt_enable_flag == GNT_ENABLE)
            {
				...
				{
					Ble_rply[10] = 0xFF;	//标签ID不匹配，执行失败
				}
            }
            else
			{
                Ble_rply[10] = 0xFE;    	//标签未使能，不支持这条指令
			}
        ...
        case 0x7111:    //终端GPS数据传递给标签
        {
            ...
            if(Data[34] == 0xFF)
            {
                gnt_lock_location((char*)&Data[10], (char*)&Data[22], 1);
            }
            else if(Data[34] == 0x00)
            {
                if(g_gnt_info.gnt_last_latitude[0] == 0)    //标签没有GPS数据
                {
                    gnt_lock_location((char*)&Data[10], (char*)&Data[22], 1);
                }
                else
...
//                        g_gnt_flash_info.gnt_report_period = Data[11]|(Data[12]<<8)|(Data[13]<<16)|(Data[14]<<24);
        ...
        {
            if((Data[10]<125)&&(Data[10]>0))
            {
                lis3dh_twi_enable();
                LIS3DH_SetInt1Threshold(Data[10]);            //设置中断阈值 16mg
                lis3dh_twi_disable();    
            }       
            Ble_rply[10] = 0x00;
            crc_v=GetCrc_16(Ble_rply, 14);
