...
/**@defgroup ble_gnts Ble gnt server.
* @{
* @ingroup app_gnt
* @brief 基于通用UUID的蓝牙通信服务. \n
* 服务包含一个特征字节，支持读、写、notify，主要用于协议通信 \n 
*/
...
#define BLE_GNTS_DEF(_name, _gnts_max_clients)                      \
    ...
    NRF_SDH_SOC_OBSERVER(_name ## _soc_obs,                         \
                         BLE_GNTS_SOC_OBSERVER_PRIO,                \
                         ble_gnts_on_sys_evt, &_name)
    #endif

    
//#define BLE_UUID_GNTS_SERVICE 0x0001    //GPS NB-iot 资产标签追踪服务UUID
#define BLE_UUID_GNTS_SERVICE 0xFFE0    //GPS NB-iot 资产标签追踪服务UUID（基于通用UUID版）
...
struct ble_gnts_s
{
    ...
    ble_gatts_char_handles_t        gnt_handles;        /**< Handles related to the GNTS characteristic (as provided by the SoftDevice). */
//    ble_gatts_char_handles_t        gnt_log_handles;
